  ...
  return <div style={{ width }}>
    <div onClick={open} style={{ cursor: "pointer", position: "relative" }}>
      ...
      {onQuickLog && <button onClick={e => { e.stopPropagation(); onQuickLog(a); }} title={listened ? "Listened" : "Mark as listened"}
        style={{ position: "absolute", top: 7, right: 7, zIndex: 3, width: 32, height: 32, borderRadius: 999, border: "none", background: listened ? "var(--accent)" : "rgba(27,26,21,.55)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <Icon name={listened ? "check" : "plus"} size={16} color="#fff" /></button>}
      {want && <div style={{ position: "absolute", top: 7, left: 7, zIndex: 3, lineHeight: 0 }}><Icon name="bookmark" size={18} color="#fff" fill="solid" /></div>}
    </div>
    <div className="display" style={{ fontSize: 14, fontWeight: 500, marginTop: 8, lineHeight: 1.2 }}>{a.title}</div>
    <div className="muted" style={{ fontSize: 12.5, marginTop: 2 }}>
      <span className="artist-link" onClick={e => { e.stopPropagation(); nav(`/artist/${encodeURIComponent(a.artist)}?aid=${a.artist_id || ""}`); }}>{a.artist}</span>{a.year ? ` · ${a.year}` : ""}
    </div>
  </div>;
}
